...
# new update
class PublicTableLookupView(APIView):
    """
    Looks up a table by its printed table_number (not the internal DB id).
    Customers only type the plain number (e.g. "1"), so this tries an exact
    match first, then falls back to matching "Table <number>" format.
    """
    permission_classes = [permissions.AllowAny]

    def get(self, request, table_number):
        raw = str(table_number).strip()

        table = RestaurantTable.objects.filter(table_number__iexact=raw).first()

        if not table:
            table = RestaurantTable.objects.filter(
                table_number__iexact=f"Table {raw}"
            ).first()

        if not table:
            return Response(
                {"detail": "No table found with that number. Please check and try again."},
                status=http_status.HTTP_404_NOT_FOUND,
            )
